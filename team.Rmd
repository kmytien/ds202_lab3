...
#Shows warnings?
dat <- readxl::read_xls('C:\\Spring2021\\ds202\\Lab3\\ds202_lab3\\GSS.xls')
```
...
```{r}
ggplot(dat1, aes(x=happiness)) + geom_bar() +theme(axis.text.x=element_text(angle = 90,hjust=1,vjust=.5)) + facet_grid(marital ~ party_affiliation)
```

Looking at the graph the relationship between marriage status, party affiliation and happiness, they all seem to have the same trend, with very happy being the most common and the count decreasing as it goes towards not too happy. 
...
The second thing we wanted to look at is if gender had an effect on happiness.
```{r}
gender_happiness <- dat1 %>%
    group_by(gender) %>%
    count(happiness) %>%
    arrange(desc(happiness))
...
Looking at this it appears that there is a larger amount of women that would be considered happier but there are actually more women than men in this data set. Therefore when you look at the percentage of Male and Female's considered to be happy, they both are right around 81%, meaning according to this data gender doesn't have much of an impact on happiness. 